...
<?= $this->section('content'); ?>
    <h3>Form Ubah Mata Kuliah</h3>

    <form action="<?= site_url('courses/update/' . $course['course_id']); ?>" method="post">
        <?= csrf_field(); ?>
        <input type="hidden" name="_method" value="PUT"> <label for="course_name">Nama Mata Kuliah:</label><br>
        <input type="text" id="course_name" name="course_name" value="<?= old('course_name', $course['course_name']); ?>" style="width: 100%; padding: 5px; margin-bottom: 10px;">
        ...
        <input type="number" id="credits" name="credits" value="<?= old('credits', $course['credits']); ?>" style="width: 100%; padding: 5px; margin-bottom: 10px;">
